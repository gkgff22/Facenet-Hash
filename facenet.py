...
def who_is_it(image, database, model):
    ...
    for (name, db_enc) in database.items():
        ...
        #print('Encoded code: %s' %(db_enc))
        print('distance for %s is %s' %(name, dist))
        #key = hashlib.sha256(("pandas" + str(np.linalg.norm(db_enc))).encode('utf-8')).hexdigest()
        #print('Key for %s is %s' %(name, key))
